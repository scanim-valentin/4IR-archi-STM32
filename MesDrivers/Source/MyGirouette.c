...
void MyGirouette_Init(MyGirouette_Struct_TypeDef * struct_girouette) {
	
	// On setup les ARR et PSC des deux compteurs A et B en format timer. Un seul timer nécessaire.
	MyTimer_Base_Init(struct_girouette->struct_compteur_AB);
	
	//////// RM 391 Encoder Interface ////////
	struct_girouette->struct_compteur_AB->Timer->TIM_SMCR = TIM_SMCR_SMS_2; // peut aussi choisir 0 et 1.
	...
	MyTimer_Base_Start(struct_girouette->struct_compteur_AB->Timer);
...
void EXTI1_IRQHandler(void) {
	...
	TIM1->SR &= ~(TIM_SR_UIF); // release interrupt
}
...
int main() {
	...
	MyGirouette_Struct_TypeDef girouette = {.Girouette_I=GPIO_I, .struct_compteur_AB=compteur_AB};
